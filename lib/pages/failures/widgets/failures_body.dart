      ...
      children: [
        ...
        Expanded(
          child: SingleChildScrollView(
            child: DataTable(
              columns: filteredSignalNames.map(
                (signalName) => DataColumn(label: Text(signalName)),
              ).toList(),
              rows: _timestamps.map(
                (timestamp) => DataRow(
                  cells: filteredSignalNames.map(
                    (signalName) => DataCell(
                      Text(_columns[signalName]![timestamp]?.toString() ?? '-'),
                    ),
                  ).toList(),
                ),
              ).toList(),
